...
const Meal = (props) => {
    ...
    var [state, setState] = useState([props.meals])
    ...
    console.log('Meal -> props.meals2', props.meals);
        ...
        // var meal = props.meals.filter(meal => meal.id == mealId[0]
        var meal = props.state.meals.find(meal => meal.id == mealId)
...
var handleEdit = () => {
    ...
    return(
        <Fragment>
        { mealId = parseInt(window.location.href.replace("http://localhost:3001/meals/", ""))},
        ...
        { meal = props.meals.find(meal => meal.id == mealId)}
            ...
            <li className='meal-component' key={props.meal} style={{listStyleType: 'none'}}>
                ...
                {
                    props.meals ? mealInfo1 : <h1>No Meal Loaded. Check api connection.</h1>
                }
            </li>
        </Fragment>
    )
